...
setInterval(async () => {
  ...
  for (const symbol of COIN_LIST) {
    try {
      ...
      const openTradesForSymbol = activePositions[symbol] ? Object.keys(activePositions[symbol]).length : 0;
      if (openTradesForSymbol >= MAX_TRADES) continue; // skip if max trades reached for this coin

      // 1H STC for direction
...
log("🤖 Multi-User Market Order Bot started.");
