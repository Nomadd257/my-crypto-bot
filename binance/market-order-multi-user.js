...
async function checkVolumeImbalance(symbol, direction) {
  try {
    ...
    for (const t of trades) {
      ...
      if (t.m) sellVolume += parseFloat(t.q); // 'q' is qty
    ...
    if (direction === "BUY") return buyAggression >= 0.6; // buy dominates last candle
    if (direction === "SELL") return sellAggression >= 0.6; // sell dominates last candle
