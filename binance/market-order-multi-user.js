...
async function executeMarketOrderForAllUsers(symbol, direction) {
  ...
  if (!emaData) {
    await sendMessage(`⚠️ EMA3 unavailable for ${symbol}`);
    return;
  }
  if (direction === "BULLISH" && emaData.lastClose < emaData.ema3) {
    await sendMessage(`⏳ Close below EMA3 for ${symbol}. Skipping.`);
    return;
  }
  if (direction === "BEARISH" && emaData.lastClose > emaData.ema3) {
    await sendMessage(`⏳ Close above EMA3 for ${symbol}. Skipping.`);
    return;
  }
  ...
  for (const { userId, client } of clients) {
    try {
      ...
      if (!bal || bal <= 0) {
        await sendMessage(`⚠️ User ${userId} has NO USDT. Skipping.`);
        continue;
      }
      ...
      if (!markPrice || markPrice <= 0) {
        log(`⚠️ markPrice invalid for ${symbol}, user ${userId}`);
        continue;
      }
...
bot.on("message", async (msg) => {
  try {
    ...
    if (String(msg.from.id) === String(ADMIN_ID) && text.toLowerCase().startsWith("/close ")) {
      ...
      const clients = createBinanceClients();
      if (!activePositions[symbolArg])
        return bot.sendMessage(chatId, `⚠️ No active position found for *${symbolArg}*`, { parse_mode: "Markdown" });
      for (const userId of Object.keys(activePositions[symbolArg])) {
        ...
        try {
          ...
          await sendMessage(`❌ Failed to close User ${userId} on ${symbolArg}: ${err?.message || err}`);
        }
      }
    ...
    const timer = setInterval(async () => {
      ...
      const emaCheck =
        (direction === "BULLISH" && emaData.lastClose > emaData.ema3) ||
        (direction === "BEARISH" && emaData.lastClose < emaData.ema3);
      if (!emaCheck) {
        await sendMessage(`⏳ Last candle close not valid for EMA3 on ${symbol}`);
        return;
      }
