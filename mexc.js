...
// ===== CONFIG =====
const TELEGRAM_BOT_TOKEN = config.TELEGRAM_BOT_TOKEN_MEXC;
...
const TOKEN_NAMES = {
  ...
  EFCUSDT: "Everton Fan Token",
  SANTOSUSDT: "Santos FC Fan Token",
  PSGUSDT: "PSG Fan Token",
...
const BB_SQUEEZE_PCT = 1.5; // Bollinger squeeze threshold
const BB_EXPAND_PCT = 3.5; // Bollinger expansion threshold
...
// ===== FETCH CANDLES =====
async function fetchSpotCandles(symbol, interval = "15m", limit = 120) {
...
/* ========== PRICE ACTION SPEED (15m, percent-change over last 24 closed candles) ========== */
...
function detectDailyDirection(symbol, candles4h) {
  ...
  sendTelegramAlert(`💠 *${tokenName}*\n🗓️ *Daily Direction* on *${symbol}*: ${dir}`);
...
function detectLiquidityAndManipulation15(symbol, candles15m) {
  ...
  const header = `💧 *${tokenName}* - Potential Fake Price Movements (15m)`;
  const body =
...
// ===== EXTENDED MANIPULATION DETECTION (15m) =====
...
function checkExtendedManipulation(symbol, candles15m) {
  try {
    ...
    if (trueCount >= 2) {
      ...
      const tokenName = TOKEN_NAMES[symbol] || symbol;
      const header = `🕵️ *Manipulation in progress* on *${tokenName}* (15m)`;
      const body =
...
/* ===== MAIN SCANNER LOOP (Spot) ===== */
async function runSpotScannerAll() {
  for (const symbol of SPOT_SYMBOLS) {
    try {
      // Fetch Spot candles
      const candles15m = await fetchSpotCandles(symbol, "15m", 120);
      const candles30m = await fetchSpotCandles(symbol, "30m", 120);
      const candles4h = await fetchSpotCandles(symbol, "4h", 6);
      ...
      console.error(`Spot scanner error for ${symbol}:`, err?.message || err);
    }
  }
}

// Start scanner
setInterval(runSpotScannerAll, RUN_INTERVAL_MS);
runSpotScannerAll();
