...
async function checkVolumeImbalance(symbol, direction) {
  try {
    ...
    const res = await fetch(`https://fapi.binance.com/fapi/v1/aggTrades?symbol=${symbol}&startTime=${startTime}&endTime=${endTime}`);
...
async function executeMarketOrderForAllUsers(symbol, direction) {
  try {
    ...
    if (!clients.length) { await sendMessage(`⚠️ No active users found.`); return; }
    ...
    for (const { userId, client } of clients) {
      try {
        ...
        let markPrice = null;
        try { const mp = await client.futuresMarkPrice(symbol); markPrice = mp.markPrice || parseFloat(mp[0]?.markPrice || 0); } catch {}
        if (!markPrice) {
  ...
  } catch(err){ log(`❌ executeMarketOrder error ${symbol}: ${err?.message || err}`); }
}
  ...
  for(const [symbol, users] of Object.entries(activePositions)){
    for(const [userId,pos] of Object.entries(users)){
      ...
      try{
        ...
        if(!p||positionAmt===0){ delete activePositions[symbol][userId]; continue; }
        ...
        // TP / SL
        const move = pos.side==="BUY" ? ((markPrice-pos.entryPrice)/pos.entryPrice)*100 : ((pos.entryPrice-markPrice)/pos.entryPrice)*100;
        if(move>=TP_PCT){
          const qty = Math.abs(positionAmt);
      ...
      }catch(err){ log(`❌ monitorPositions error for ${userId} ${symbol}: ${err?.message||err}`); }
    }
  }
}
