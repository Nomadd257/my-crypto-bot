// ===== PART 1/3 =====
// Imports, config, state, helpers

const axios = require("axios");
const technicalIndicators = require("technicalindicators");
const { EMA, Stochastic, MACD, ATR } = technicalIndicators;
const fs = require("fs");
const config = require("../config");
// ===== CONFIG =====
const TELEGRAM_BOT_TOKEN = config.TELEGRAM_BOT_TOKEN_BYBIT_FVG;
...
// Price Action Speed using % change thresholds
const PRICE_ACTION_LOOKBACK = 24; // number of CLOSED 15m candles (~6 hours)
const PRICE_ACTION_THRESH_PCT = 1.3; // percent threshold
...
let lastPriceActionAlert = {}; // dedupe for price action speed alerts
let lastExtendedManipAlert = {}; // dedupe / cooldown for extended manipulation alerts
...
/* Convert typical intervals to Bybit's interval parameter */
function toBybitInterval(interval) {
  if (!interval) return "15";
  const s = interval.toString().trim().toLowerCase();
  ...
  if (s.endsWith("m")) return s.replace(/m$/i, "");
  if (s.endsWith("h")) return String(parseInt(s.replace(/h$/i, ""), 10) * 60);
  if (s.toUpperCase() === "D") return "D";
  return s;
}

/* Fetch candles from Bybit v5 market/kline (category=linear) */
async function fetchCandles(symbol, interval, limit = 120) {
  ...
  if (!res.data) throw new Error("Bybit response empty");
  if (res.data.retCode && res.data.retCode !== 0) {
    throw new Error("Bybit kline error: " + JSON.stringify(res.data));
  }
  const list = (res.data.result && res.data.result.list) || [];
  // Bybit may return newest-first; ensure earliest -> latest
...
function detectLiquiditySweepGeneric(candles, lookback = SWEEP_LOOKBACK) {
  ...
  const lastClosed = candles[candles.length - 2]; // only consider closed candle
  const ref = candles.slice(-(lookback + 2), -2);
  const priorHigh = Math.max(...ref.map((c) => c.high));
  const priorLow = Math.min(...ref.map((c) => c.low));

  const upSweep = lastClosed.high > priorHigh && lastClosed.close < priorHigh && lastClosed.close < lastClosed.open;
  const downSweep = lastClosed.low < priorLow && lastClosed.close > priorLow && lastClosed.close > lastClosed.open;

  if (upSweep) return { side: "up", time: lastClosed.time };
  if (downSweep) return { side: "down", time: lastClosed.time };
  return null;
}
...
function estimateMoveAndTimeFrom30(candles30m, dir, pressureLabel) {
  ...
  const price = candles30m[candles30m.length - 2].close; // last closed candle
...
// ===== PART 2/3 =====
// Cycle alerts + Extended Manipulation detection
...
const EXT_MANIP_WHIPSAW_PCT = 4; // Whipsaw threshold (4%)
const EXT_MANIP_VOL_FACTOR = 2; // Volume spike factor (2× avg)
...
function checkExtendedManipulation(symbol, candles15m) {
  try {
    ...
    const now = Date.now();
    const lastAlertTime = lastExtendedManipAlert[symbol] || 0;
    if (now - lastAlertTime < EXT_MANIP_COOLDOWN_MS) return;

    // Determine session (Asia / London / New York)
    ...
    if (pctHighFromLow >= EXT_MANIP_WHIPSAW_PCT) {
      ...
      if (Math.abs(idxHigh - idxLow) >= 1) {
        const upThenDown =
          (priceHigh / priceLow - 1) * 100 >= EXT_MANIP_WHIPSAW_PCT &&
          ((priceHigh - priceLow) / priceHigh) * 100 >= EXT_MANIP_WHIPSAW_PCT;
        if (upThenDown) whipsaw = true;
      }
    } else {
      // fallback: local pair swings >4% up then >4% down (or vice versa)
      for (let i = 1; i < closes.length - 1; i++) {
        const upPct = ((closes[i] - closes[i - 1]) / closes[i - 1]) * 100;
        const downPct = ((closes[i + 1] - closes[i]) / closes[i]) * 100;
        if (
          (upPct >= EXT_MANIP_WHIPSAW_PCT && downPct <= -EXT_MANIP_WHIPSAW_PCT) ||
          (upPct <= -EXT_MANIP_WHIPSAW_PCT && downPct >= EXT_MANIP_WHIPSAW_PCT)
        ) {
          whipsaw = true;
          break;
    ...
    // 2) High-volume clusters: count spikes and check price-range clustering
    const avgVol =
    ...
    if (spikeIdxs.length >= EXT_MANIP_MIN_SPIKES) {
      ...
      if (bandPct <= 1.0) volumeCluster = true; // clustered in ~1% band
    }

    // 3) STC flips multiple times (choppy)
    ...
    const stcChoppy = flips >= EXT_MANIP_STC_FLIPS; // 5+ flips

    // decide: require at least two characteristics to trigger
    const detected = [whipsaw, volumeCluster, stcChoppy];
    const trueCount = detected.filter(Boolean).length;
    if (trueCount >= 2) {
      const lastClosed = closed[closed.length - 1];
      lastExtendedManipAlert[symbol] = now;

      // Build message listing detected characteristics
      const charList = [];
      ...
      const body =
        `⚠️ Fake price movements detected.\n` +
        `Session: *${session}*\n\n` +
...
// ===== PART 3/3 =====
// Liquidity manipulation, Price Action Speed (percent change), daily direction, main scanner loop

/* ========== LIQUIDITY / MANIPULATION (15m) ========== */
function detectLiquidityAndManipulation15(symbol, candles15m) {
  if (!candles15m || candles15m.length < 30) return;
  // Use last CLOSED candle for analysis
  const lastClosed = candles15m[candles15m.length - 2];
  const price = lastClosed.close;
  const lookback = candles15m.slice(-30, -1); // 30 bars excluding current
  ...
  const expectedSweepBuffer = price * 0.005; // ~0.5%
  ...
  // dedupe by symbol + candle-time
  const dedupKey = `${symbol}_${lastClosed.time}`;
...
/* ========== PRICE ACTION SPEED (15m) - percent change over closed candles ========== */
function detectPriceActionSpeed15(symbol, candles15m) {
  // Requires PRICE_ACTION_LOOKBACK and PRICE_ACTION_THRESH_PCT defined in Part 1
  if (!candles15m || candles15m.length < PRICE_ACTION_LOOKBACK + 1) return;

  const closed = candles15m.slice(0, -1); // use closed candles only
  if (closed.length < PRICE_ACTION_LOOKBACK) return;

  const window = closed.slice(-PRICE_ACTION_LOOKBACK);
  const oldest = window[0].close;
  const latest = window[window.length - 1].close;
  const pctChange = ((latest - oldest) / oldest) * 100;

  const lastClosed = closed[closed.length - 1];
  const dedupKey = `${symbol}_${lastClosed.time}`;
  if (lastPriceActionAlert[dedupKey]) return;

  // Squeeze if |change| <= 1.3%; Expansion if |change| > 1.3% (with direction)
  if (Math.abs(pctChange) <= PRICE_ACTION_THRESH_PCT) {
    lastPriceActionAlert[dedupKey] = true;
    const msg =
      `📊 *Price Action Speed* on *${symbol}* (15m)\n` +
      `• Change: ${pctChange.toFixed(2)}%\n` +
      `• *Squeeze* (Price movements slowing down)`;
    sendTelegramAlert(msg);
  } else if (Math.abs(pctChange) > PRICE_ACTION_THRESH_PCT) {
    lastPriceActionAlert[dedupKey] = true;
...
/* ========== DAILY DIRECTION (first 4 UTC hours) ========== */
async function detectDailyDirection(symbol) {
  try {
    const today = new Date().toISOString().split("T")[0];
    if (lastDailyDirectionSent[symbol] === today) return;

    // Fetch 1-hour candles (last 24h) to inspect the first 4 UTC hours
    const candles1h = await fetchCandles(symbol, "60", 24);
    ...
    const startOfDayUTC = Date.UTC(y, m, d, 0, 0, 0, 0);

    // Select candles that fall into the first 4 UTC hours
    const first4 = candles1h.filter((c) => c.time >= startOfDayUTC && c.time < startOfDayUTC + 4 * 3600000);

    // Fallback: if we couldn't find first4 by timestamp, take the earliest 4 from the fetched set
    const first4Used = first4 && first4.length >= 1 ? first4 : candles1h.slice(0, 4);
    if (!first4Used || first4Used.length < 1) return;

    const open = first4Used[0].open;
    const close = first4Used[first4Used.length - 1].close;
    ...
    console.error(`detectDailyDirection error for ${symbol}:`, err?.message || err);
  }
}

/* ========== MAIN BYBIT SCANNER LOOP ========== */
async function runBybitScanner() {
  for (const symbol of SYMBOLS) {
    try {
      // Fetch candles for symbol (Part 1's fetchCandles handles Bybit interval conversion)
      ...
      // Ensure candles have enough data
      if (!candles15m?.length || !candles30m?.length || !candles4h?.length) {
        console.warn(`Insufficient candles for ${symbol}`);
        continue;
      }

      // --- PART 2: CYCLE DETECTION (functions in Part 2) ---
      ...
      // Extended manipulation (Part 2)
      checkExtendedManipulation(symbol, candles15m);

      // --- PART 3: SCANNER ALERTS ---
      detectLiquidityAndManipulation15(symbol, candles15m);
      detectPriceActionSpeed15(symbol, candles15m);
      // Daily direction checks first 4 UTC hours (internal fetch) and runs once per UTC day
      await detectDailyDirection(symbol);
    } catch (err) {
...
setInterval(runBybitScanner, RUN_INTERVAL_MS);
runBybitScanner();
